function [energy, tp, tn, fp, fn, num_mal, std_norm, mean_norm, std_mal, mean_mal,t]=multiBiometric_conf(myReward, myPunish, myThreshold, dispMode)
    ...
    energy_cnt=1;                           %how many energy is set
    ...
    giveUp=0.4e+3;                                                    %when there is no change I give up
    resolution=50;
    ...
    while (t<giveUp)
        ...
        if(or(start==0,t>(energy_cnt-1)*resolution))             %energy count fro record
            energy_track(energy_cnt)=nd_energy;             
            time_track(energy_cnt)=t;
            energy_cnt=energy_cnt+1; 
            start=1;
        end
...
%         end
    ...
    if (dispMode==1)
        ...
        hold on
%         if(myThreshold==0.1)
%             plot (time_track, energy_track, '-ob', 'linewidth', 2);    
%         elseif(myThreshold==0.25)
%             plot (time_track, energy_track, '-sr', 'linewidth', 2); 
%         elseif(myThreshold==0.5)
%             plot (time_track, energy_track, '-*c', 'linewidth', 2); 
%         elseif(myThreshold==0.75)
%             plot (time_track, energy_track, '-^g', 'linewidth', 2);
%         elseif(myThreshold==1.0)
%             plot (time_track, energy_track, '-vk', 'linewidth', 2); 
%         else
%              plot (time_track, energy_track, '-.k', 'linewidth', 2);
%             legend('\gamma=0.10','\gamma=0.25','\gamma=0.50','\gamma=0.75','\gamma=1.00','\gamma=1.10');
%         end
        plot (time_track, energy_track, 'linewidth', 2);
        xlabel('Time (s)');
        ylabel('Energy (J)');
        title('Time vs Energy Consumed');
        txt=sprintf('\\leftarrow \\gamma=%0.2f', threshold);
        text(time_track(end), energy_track(end),txt);
        %xlim([0 giveUp+giveup/5])
        grid on;
        hold off;
